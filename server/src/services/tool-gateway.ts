...
} from "./tool-connection-headers.js";
import {
...
export function createToolGatewayService(
  ...
  async function callRemoteConnectionProtocol(input: {
    ...
    const { headers } = buildRemoteHeaders({
      ...
      projectedHeaders: projectedConnectionHeaders(input.connection),
      credentialHeaders,
  ...
  async function executeRemoteHttpTool(
    ...
    let credentialHeaders = await resolveCredentialHeaders(session, connection, grant);
    ...
    try {
      ...
      if (
        ...
        credentialHeaders = await resolveCredentialHeaders(session, connection, grant, {
          forceRefresh: true,
        });
