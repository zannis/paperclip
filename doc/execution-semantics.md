...
Because that reachability is a precondition and not a per-run choice, the server validates it once at startup rather than letting each run discover it. As soon as the listener is up, Paperclip probes `GET /api/health` against the resolved runtime API URL and requires a response carrying that route's own shape — a `status` of `ok` or `unhealthy` plus at least one field only that route serves. Any HTTP status is acceptable, including `503 unhealthy` — the probe establishes that the board API is on the other end, not that it is healthy. A redirect or an HTML body means the URL names a UI origin or an auth proxy rather than the API, and the server logs the rejected URL with the reason and falls back to its own bound origin if that answers. If nothing answers, the server keeps the configured value and logs the failure: external clients may reach an origin the server process cannot, so taking the API down is the worse outcome. Runs spawned in the window before the listener comes up can still inherit an unvalidated URL.

The probe has exactly two candidates, and they are not interchangeable, because a spawned run attaches its bearer run token to whatever origin wins. The configured URL is the operator's explicit choice and is what every run inherited before this check existed, so probing it can only reject it — never widen where credentials go — and the board health shape is signal enough to keep it. The fallback is chosen by the server, so promoting it is the only way this check could send a run somewhere new, and no response body can prove that the responder is this server: every self-identifying value the health route publishes is readable off that same route by anything that can reach it, so any process able to answer the probe can also echo whatever the probe wants to see. The fallback is therefore authenticated by construction rather than by content — it is the address the process actually bound (its family's loopback address for a wildcard bind, since that bind holds the whole port), an origin no other process can be holding. There is no second fallback, because there is no second origin that can be proven to be the server. A listener with no HTTP origin has no fallback at all, and a failing configured URL is left in place, logged. The wider runtime API candidate list is unaffected — runtime clients still walk it at request time; it just cannot supply the credential-bearing default.

`PAPERCLIP_API_URL` resolves highest-precedence-first as: the startup-validated server-level `PAPERCLIP_API_URL`, then `PAPERCLIP_RUNTIME_API_URL`, then the derived loopback origin. The startup probe is the recovery path for a wrong runtime URL; adapter and user config are not. `PAPERCLIP_API_URL` stays inside the reserved `PAPERCLIP_*` namespace that config may never override once Paperclip has assigned it for the run, even though the URL is routing rather than a credential: an agent-authenticated caller can patch its own `adapterConfig`, so honouring the key from config would let an agent redirect its harness-minted run token to an origin of its choosing. `PAPERCLIP_API_KEY` is likewise never accepted from adapter or user config — the run token is its only source.
