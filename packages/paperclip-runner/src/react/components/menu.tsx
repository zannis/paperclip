...
export function Menu({
  ...
  return (
    <div ref={containerRef} data-slot="menu" className="pcr-menu">
      <button
        ref={triggerRef}
        id={triggerId}
        type="button"
        className="pcr-button pcr-button--quiet"
        aria-haspopup="menu"
        aria-expanded={open}
      ...
      {disabled && disabledReason !== null ? (
        <p id={descriptionId} className="pcr-menu-reason">
          {disabledReason}
        </p>
      ) : null}
      {open && !disabled ? (
        <div role="menu" aria-labelledby={triggerId} className="pcr-menu-list" onKeyDown={onMenuKeyDown}>
          {items.map((item) => {
            ...
            return (
              <React.Fragment key={item.id}>
                ...
                {item.reason === null ? null : (
                  <p id={itemReasonId} className="pcr-menu-reason">
                    {item.reason}
                  </p>
                )}
              </React.Fragment>
